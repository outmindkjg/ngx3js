...
export abstract class AbstractEffectComposer {
    abstract resetEffectComposer():void;
}
...
export class ThreeUtil {
    ...
    static getColorSafe(color: string | number | THREE.Color, altColor?: string | number | THREE.Color): THREE.Color {
    ...
    static getVector2Safe<T>(x: number, y: number, altValue?: THREE.Vector2): THREE.Vector2 {
        const defValue = this.isNotNull(x) && this.isNotNull(y) ? new THREE.Vector2(x, y) : altValue;
        if (this.isNotNull(defValue)) {
            return defValue;
        }
        return undefined;
    ...
    static getVector3Safe<T>(x: number, y: number, z: number, altValue?: THREE.Vector3): THREE.Vector3 {
        const defValue = this.isNotNull(x) && this.isNotNull(y) && this.isNotNull(z) ? new THREE.Vector3(x, y, z) : altValue;
        if (this.isNotNull(defValue)) {
            return defValue;
        }
        return undefined;
    ...
    static setupGuiChange(control: ThreeGuiController, onFinishChange?: (value?: any) => void, onChange?: (value?: any) => void, listen?: boolean) {
