...
export  class TextureUtils {
  static scale(w : number , h : number, ow : number, oh : number): THREE.Vector2 {
    return new THREE.Vector2(w/ow, h/oh);
  }

  static addCanvas(key : string , canvas : CanvasFunctionType) {
    CanvasConf[key.toLowerCase()] = canvas;   
  }

  static getCanvas(key : string | CanvasFunctionType) :CanvasFunctionType {
  ...
  static addDataTexture(key : string , data : DataFunctionType) {
    DataTextureConf[key.toLowerCase()] = data;   
  }

  static getDataTexture(key : string | DataFunctionType) :DataFunctionType {
    if (key === null || key === undefined) {
      return null;
    } else if (typeof key === 'string') {
