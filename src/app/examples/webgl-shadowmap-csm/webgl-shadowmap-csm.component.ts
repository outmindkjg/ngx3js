...
export class WebglShadowmapCsmComponent extends BaseComponent<{
	...
	constructor() {
		super(
			...
			[
				...
				{
					name: 'mode',
					title: 'frustum split mode',
					type: 'select',
					select: ['uniform', 'logarithmic', 'practical'],
					change: () => {
						if (this.csm !== null) {
	...
	changeHelper() {
		if (this.helper !== null) {
			...
			this.helper.displayShadowBounds =
				this.controls.helper.displayShadowBounds;
