  ...
  constructor() {
    super({
      ...
      { name : 'output', type : 'select', select : {
        ...
        'Depth': 3,
        'Normal': 4
      }, change : () => { this.changeSaoParam();}},
      { name : 'saoBias', type : 'number', min : -1, max : 1, change : () => { this.changeSaoParam();}},
      { name : 'saoIntensity', type : 'number', min : 0, max : 1, change : () => { this.changeSaoParam();}},
      { name : 'saoScale', type : 'number', min : 0, max : 10, change : () => { this.changeSaoParam();}},
      { name : 'saoKernelRadius', type : 'number', min : 1, max : 100, change : () => { this.changeSaoParam();}},
      { name : 'saoMinResolution', type : 'number', min : 0, max : 1, change : () => { this.changeSaoParam();}},
      { name : 'saoBlur', type : 'checkbox', change : () => { this.changeSaoParam();}},
      { name : 'saoBlurRadius', type : 'number', min : 0, max : 200, change : () => { this.changeSaoParam();}},
      { name : 'saoBlurStdDev', type : 'number', min : 0.5, max : 150, change : () => { this.changeSaoParam();}},
      { name : 'saoBlurDepthCutoff', type : 'number', min : 0.0, max : 0.1, change : () => { this.changeSaoParam();}},
    ]);
  }
