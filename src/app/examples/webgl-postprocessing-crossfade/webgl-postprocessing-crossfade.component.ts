...
export class WebglPostprocessingCrossfadeComponent extends BaseComponent<{
  ...
  constructor() {
    super({
      ...
      { name : "texture", type : "select", select : { Perlin: 0, Squares: 1, Cells: 2, Distort: 3, Gradient: 4, Radial: 5 }},
      { name : "textureThreshold", type : "number", min : 0, max : 1, step : 0.01 },
      { name : "animateTransition", type : "checkbox"},
      { name : "transition", type : "number", min : 0, max : 1, step : 0.01 },
      { name : "transitionSpeed", type : "number", min : 0.5, max : 5, step : 0.01 },
    ]);
  }
