...
export class WebglInteractiveRaycastingPointsComponent extends BaseComponent<{}> {
	...
	ngOnInit() {
		const width = 80;
		const length = 160;
		this.pointCloudInfo = this.generatePointCloudGeometry(
			{ r: 1, g: 0, b: 0 },
			width,
			length
		);
		this.indexedPointcloud = this.generatePointCloudGeometry(
			{ r: 0, g: 1, b: 0 },
			width,
			length
		);
		...
		this.indexedWithOffsetPointcloud = this.generatePointCloudGeometry(
			{ r: 1, g: 1, b: 1 },
			width,
			length
		);
