...
export class WebglMaterialsSubsurfaceScatteringComponent extends BaseComponent<{
  ...
  constructor() {
    super({
      ...
      { name : 'distortion', type : 'number', min : 0.01, max : 1, step : 0.01, change : () => { 
        this.updateUniforms('thicknessDistortion', this.controls.distortion )
      }},
      { name : 'ambient', type : 'number', min : 0.01, max : 5, step : 0.05, change : () => { 
        this.updateUniforms('thicknessAmbient', this.controls.ambient )
      }},
      { name : 'attenuation', type : 'number', min : 0.01, max : 5, step : 0.05, change : () => { 
        this.updateUniforms('thicknessAttenuation', this.controls.attenuation )
      }},
      { name : 'power', type : 'number', min : 0.01, max : 16, step : 0.1, change : () => { 
        this.updateUniforms('thicknessPower', this.controls.power )
      }},
      { name : 'scale', type : 'number', min : 0.01, max : 50, step : 0.1, change : () => { 
  ...
  setMatrial(matrial : MaterialComponent) {
    this.matrial = matrial.getMaterial() as ShaderMaterial;
  }

  matrial : ShaderMaterial = null;
