...
export class Css3dPeriodictableComponent extends BaseComponent<{
  ...
  ngOnInit() {
    ...
    for (let i = 0; i < table.length; i += 5) {
      ...
      this.periodicInfo.push({
        name : table[i],
        innerHtml: `
        <div class='number'>${i / 5 + 1}</div>  
        <div class='symbol'>${table[i]}</div>  
        <div class='details'>${table[i + 1]}<br>${table[i + 2]}</div>  
        `,
  ...
  changeTable() {
    if (this.mesh !== null && this.meshChildren && this.meshChildren.length > 0) {
      let baseTable: { position: THREE.Vector3; rotation: THREE.Euler }[] = null;
      switch (this.controls.display) {
        case 'table':
          baseTable = this.tweenTable.table;
          break;
        case 'sphere':
          baseTable = this.tweenTable.sphere;
          break;
        case 'helix':
          baseTable = this.tweenTable.helix;
          break;
        case 'grid':
          baseTable = this.tweenTable.grid;
          break;
      }
      if (baseTable !== null) {
        let seqn = 0;
        this.meshChildren.forEach(child => {
          if (child.name) {
            const currData = baseTable[seqn] || { position : null , rotation : null };
            child.userData.tween = {
              elapsedTime: 0,
              elapsedAlpha : 0,
              position : currData.position,
              rotation : currData.rotation,
            }
            seqn++;
          }
        });
