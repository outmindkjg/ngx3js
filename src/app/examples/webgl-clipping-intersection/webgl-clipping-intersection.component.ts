...
export class WebglClippingIntersectionComponent extends BaseComponent<{
  ...
  changeClipIntersection(mesh : THREE.Object3D) {
    if (mesh instanceof THREE.Mesh && mesh.material && mesh.material instanceof THREE.Material) {
  ...
  ngOnInit() {
    ...
    for ( let i = 1; i <= 30; i += 2 ) {
      this.sphereInfos.push({ color : new THREE.Color().setHSL( Math.random(), 0.5, 0.5 ).getHex(), radius : i / 30});
   }
  }
