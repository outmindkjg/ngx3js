...
export class WebglMultipleRenderersComponent extends BaseComponent<{
  ...
  layoutFirst: { x: string | number; y: string | number; width: string | number; height: string | number } = null;
  layoutSecond: { x: string | number; y: string | number; width: string | number; height: string | number } = null;

  setGeometry1( geometry : THREE.IcosahedronGeometry) {
  ...
  setGeometry2( geometry : THREE.IcosahedronGeometry) {
    const count = geometry.attributes.position.count;
    geometry.setAttribute( 'color', new THREE.BufferAttribute( new Float32Array( count * 3 ), 3 ) );
    const positions = geometry.attributes.position;
    const colors = geometry.attributes.color;
  ...
  setGeometry3( geometry : THREE.IcosahedronGeometry) {
    const count = geometry.attributes.position.count;
    geometry.setAttribute( 'color', new THREE.BufferAttribute( new Float32Array( count * 3 ), 3 ) );
    const positions = geometry.attributes.position;
    const colors = geometry.attributes.color;
