...
export class WebglShadersSkyComponent extends BaseComponent<{
  ...
  constructor() {
    super({
      ...
      { name : 'mieCoefficient', type : 'number', min : 0.0, max : 0.1, step : 0.001 },
      { name : 'mieDirectionalG', type : 'number', min : 0.0, max : 1, step : 0.001 },
      { name : 'inclination', type : 'number', min : 0.0, max : 1, step : 0.0001, finishChange : () => {
        this.updateSun();
      }},
      { name : 'azimuth', type : 'number', min : 0.0, max : 1, step : 0.0001, finishChange : () => {
        this.updateSun();
      }},
