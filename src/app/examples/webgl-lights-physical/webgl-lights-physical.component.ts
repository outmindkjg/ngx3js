...
export class WebglLightsPhysicalComponent extends BaseComponent<{
  ...
  constructor() {
    super({
      ...
      { name : 'hemiIrradiance', type : 'select', select : [
      ...
      { name : 'bulbPower', type : 'select', select : [
        "110000 lm (1000W)",
        "3500 lm (300W)",
        "1700 lm (100W)",
        "800 lm (60W)",
        ...
        if (this.bulbLight !== null) {
          this.bulbLight.power = this.bulbLuminousPowers[ this.controls.bulbPower ];
        }
      }},
      { name : 'exposure', type : 'number', min : 0, max : 1 , change : () => {
        if (this.renderer !== null) {
          (this.renderer.getRenderer() as any).toneMappingExposure = Math.pow( this.controls.exposure, 5.0 ); 
          // to allow for very bright scenes.
        }
      }},
      { name : 'shadows', type : 'checkbox', change : () => {
        if (this.bulbLight !== null) {
  ...
  setRender(renderer : RendererComponent) {
    ...
    const render = renderer.getRenderer() as THREE.WebGLRenderer;
