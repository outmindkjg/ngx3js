...
export class WebglClippingComponent extends BaseComponent<{
  ...
  constructor() {
    super({
      ...
      { name : 'Local Clipping', type : 'folder', control : 'localClipping', children : [
        { name : 'enabled', title : 'Enabled', type : 'checkbox', change : () => { 
          if (this.renderer !== null) {
            const renderer = this.renderer.getRenderer() as THREE.WebGLRenderer;
            renderer.localClippingEnabled = this.controls.localClipping.enabled;
          }
        }},
        ...
        { name : 'plane', title : 'Plane', type : 'number', min : -0.5, max : 0.5, change : () => { 
          if (this.localPlane !== null) {
            this.localPlane.setPlane(null, null, null, this.controls.localClipping.plane);
          }
        }}
      ], isOpen : true},
      { name : 'Global Clipping', type : 'folder', control : 'globalClipping', children : [
        { name : 'enabled', title : 'Enabled', type : 'checkbox', change : () => { 

        }},
        { name : 'plane', title : 'Plane', type : 'number', min : -0.4, max : 3, change : () => { 
          if (this.globalPlane !== null) {
            this.globalPlane.setPlane(null, null, null, this.controls.globalClipping.plane);
          }
        }}
      ], isOpen : true},
  ...
  onRender(timer : RendererTimer) {
    ...
    if (this.mesh !== null) {
      ...
      this.mesh.setScaleScalar(
        Math.cos( timer.elapsedTime ) * 0.125 + 0.875
      )
    }
