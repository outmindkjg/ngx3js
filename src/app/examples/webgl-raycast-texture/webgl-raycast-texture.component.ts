...
export class WebglRaycastTextureComponent extends BaseComponent<{
			...
			[
				{
					...
					title: 'WrapS',
					type: 'select',
					select: ['ClampToEdge', 'Repeat', 'MirroredRepeat'],
				},
				{
					name: 'wrapT',
					title: 'WrapT',
	...
	draw() {
		if (this.canvas.image !== null && this.canvas.texture != null) {
			...
			context2D.beginPath();
			context2D.moveTo(
				this.canvas.xCross - this.canvas.crossMax - 2,
				this.canvas.yCross - this.canvas.crossMax - 2
			);
			context2D.lineTo(
				this.canvas.xCross - this.canvas.crossMin,
				this.canvas.yCross - this.canvas.crossMin
			);
