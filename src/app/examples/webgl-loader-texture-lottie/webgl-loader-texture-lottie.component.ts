...
export class WebglLoaderTextureLottieComponent extends BaseComponent<{
	...
	constructor() {
		super({
			...
			{ name : 'scrubber', type : 'number', min : 0, max : 100, listen : true, change : () => {
				if (this.animation !== null) {
					this.animation.goToAndStop( this.controls.scrubber / 100 * this.totalFrames , true );
				}
			}},
		]);
	}
	...
	setLottie( texture : TextureComponent) {
		...
		this.animation?.addEventListener( 'enterFrame', () => {
			this.controls.scrubber = this.animation.currentFrame / this.totalFrames * 100;
		} );
