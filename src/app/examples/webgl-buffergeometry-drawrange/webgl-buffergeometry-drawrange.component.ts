...
export class WebglBuffergeometryDrawrangeComponent extends BaseComponent<{
  ...
  particlesData : { velocity : Vector3, numConnections : number} [] = [];  

  setLineMesh(mesh : MeshComponent) {
    this.linesMesh = mesh.getMesh();
  }

  linesMesh : LineSegments = null;
  ...
  onRender(timer : RendererTimer) {
    ...
    if (this.meshObject3d !== null && this.pointCloud !== null && this.linesMesh !== null) {
      ...
      for ( let i = 0; i < particleCount; i ++ ) {
        ...
        if ( this.controls.limitConnections && particleData.numConnections >= this.controls.maxConnections )
          continue;
        ...
        for ( let j = i + 1; j < particleCount; j ++ ) {
          ...
          if ( this.controls.limitConnections && particleDataB.numConnections >= this.controls.maxConnections )
            continue;
