...
export class WebglLightprobeComponent extends BaseComponent<{
  ...
  constructor() {
    super({
      ...
      { name : 'lightProbeIntensity', title : 'light probe', type : 'number', min : 0, max : 1, step : 0.02, change : () => {
        if (this.lightProbe !== null) {
          this.lightProbe.intensity = this.controls.lightProbeIntensity;
        }
      }},
      { name : 'directionalLightIntensity', title : 'directional light', type : 'number', min : 0, max : 1, step : 0.02, change : () => {
        if (this.directionalLight !== null) {
          this.directionalLight.intensity = this.controls.directionalLightIntensity;
        }
      }},
      { name : 'envMapIntensity', title : 'envMap', type : 'number', min : 0, max : 1, step : 0.02, change : () => {
