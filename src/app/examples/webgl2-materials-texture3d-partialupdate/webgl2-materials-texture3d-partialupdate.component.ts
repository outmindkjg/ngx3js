...
export class Webgl2MaterialsTexture3dPartialupdateComponent extends BaseComponent<{
	...
	onRender(timer: RendererTimer) {
		...
		if (this.camera !== null && this.meshObject3d !== null) {
			...
			const uniforms = this.getUniforms();
			if (uniforms !== null) {
				if (this.nextTime < timer.elapsedTime && this.addedCnt < this.texture3dList.length) {
					let i = this.addedCnt;
					const x = i % 4;
					i = Math.floor((i - x) / 4);
					const y = i % 4;
					...
					const box = new THREE.Box3( new THREE.Vector3( 0, 0, 0 ), new THREE.Vector3( 29, 29, 29 ) );
					(timer.renderer as any).copyTextureToTexture3D( box, position, texture, uniforms.map.value );
					this.addedCnt++;
