...
export class Webgl2VolumeCloudComponent extends BaseComponent<{
  ...
  constructor() {
    super({
      ...
      { name : 'opacity', type : 'number', min : 0, max : 1, step : 0.01, change : () => { this.updateUniforms(); }},
      { name : 'range', type : 'number', min : 0, max : 1, step : 0.01, change : () => { this.updateUniforms(); }},
      { name : 'steps', type : 'number', min : 0, max : 200, step : 1, change : () => { this.updateUniforms(); }}
    ]);
  }
  ...
  getUniforms() : { [uniform: string]: THREE.IUniform } {
    if (this.meshObject3d !== null) {
      const rawShaderMaterial = (this.meshObject3d as THREE.Mesh).material as THREE.RawShaderMaterial;
