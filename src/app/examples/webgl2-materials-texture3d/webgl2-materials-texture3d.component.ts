...
export class Webgl2MaterialsTexture3dComponent extends BaseComponent<{
  ...
  constructor() {
    super({
      clim1: 0, clim2: 1, renderstyle: 'iso', isothreshold: 0.15, colormap: 'viridis'
    },[
      { name : 'clim1', type : 'number', min : 0, max : 1, step : 0.01, change : () => {
        this.updateUniforms();
      }},
      { name : 'clim2', type : 'number', min : 0, max : 1, step : 0.01, change : () => {
        this.updateUniforms();
      }},
      { name : 'colormap', type : 'select', select : ['gray','viridis'], change : () => {
        this.updateUniforms();
      }},
      { name : 'renderstyle', type : 'select', select : ['mip','iso'], change : () => {
        this.updateUniforms();
      }},
      { name : 'isothreshold', type : 'number', min : 0, max : 1, step : 0.01, change : () => {
        this.updateUniforms();
      }}
    ]);
  }
  ...
  updateUniforms() {
    if (this.mesh !== null) {
      ...
      if (material !== null && material !== undefined) {
        ...
        material.uniforms[ "u_renderthreshold" ].value = this.controls.isothreshold; // For ISO renderstyle
