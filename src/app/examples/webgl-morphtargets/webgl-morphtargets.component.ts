...
export class WebglMorphtargetsComponent extends BaseComponent<{
		...
		if (
			...
			for (let i = 0; i < positionAttribute.count; i++) {
				...
				spherePositions.push(
					x *
						Math.sqrt(
							Math.abs(1 - (y * y) / 2 - (z * z) / 2 + (y * y * z * z) / 3)
						),
					y *
						Math.sqrt(
							Math.abs(1 - (z * z) / 2 - (x * x) / 2 + (z * z * x * x) / 3)
						),
					z *
						Math.sqrt(
							Math.abs(1 - (x * x) / 2 - (y * y) / 2 + (x * x * y * y) / 3)
						)
				);
				vertex.set(x * 2, y, z);
				vertex
	...
	onRender(timer: RendererTimer) {
		...
		if (
			...
			this.threeMesh.morphTargetInfluences &&
			this.threeMesh.morphTargetInfluences.length > 1
		) {
			const time = timer.elapsedTime;
			this.threeMesh.morphTargetInfluences[0] =
				0.5 + Math.sin(time * 0.6) * 0.5;
			this.threeMesh.morphTargetInfluences[1] =
				0.5 + Math.cos(time * 0.7) * 0.5;
		}
	}
}
