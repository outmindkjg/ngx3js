...
import { BaseComponent, BufferGeometryUtils, MeshComponent, RendererTimer, THREE } from 'ngx3js';
...
export class WebglBuffergeometryCompressionComponent extends BaseComponent<{
  ...
  constructor() {
    super({
      ...
      { name : 'Scene', type : 'folder', children : [
        { name : 'model', type : 'select', select : ["Icosahedron", "Cylinder", "TorusKnot", "Teapot"], change : () => {
          this.changeModel();
        }},
        { name : 'wireframe', type : 'checkbox', change : () => {
          this.changeModel();
        }},
        { name : 'texture', type : 'checkbox', change : () => {
          this.changeModel();
        }},
        { name : 'detail', type : 'number', min : 1, max : 8, step : 1, change : () => {
          this.changeModel();
        }},
        { name : 'rotationSpeed', type : 'number', min : 0, max : 0.5, step : 0.1, change : () => {
          this.changeModel();
        }}
      ], isOpen : true},
      { name : 'Position Compression', type : 'folder', children : [
        { name : 'QuantizePosEncoding', type : 'checkbox', change : () => {
          this.changeModel();
        }},
      ], isOpen : true},
      { name : 'Normal Compression', type : 'folder', children : [
        { name : 'NormEncodingMethods', type : 'select', select : ["None", "DEFAULT", "OCT1Byte", "OCT2Byte", "ANGLES"], change : () => {
          this.changeModel();
        }},
      ], isOpen : true},
      { name : 'UV Compression', type : 'folder', children : [
        { name : 'DefaultUVEncoding', type : 'checkbox', change : () => {
          this.changeModel();
        }},
      ], isOpen : true},
      { name : 'Memory Info', type : 'folder', children : [
        { name : 'totalGPUMemory', type : 'input', listen : true },
      ], isOpen : true},
    ]);
  }
  ngOnInit() {
  ...
  onRender(timer : RendererTimer) {
    ...
    this.lights.forEach( light => {
      ...
      direction.applyAxisAngle( new THREE.Vector3( 1, 1, 0 ), this.controls.rotationSpeed / 180 * Math.PI );
