...
export class WebglCameraCinematicComponent extends BaseComponent<{
  ...
  constructor() {
    super({
      ...
      { name : 'focalLength', type : 'number', listen : true, min : 1, max : 135, step : 0.01, change : () => { this.matChanger(); }},
      { name : 'fstop', type : 'number', listen : true, min : 1.8, max : 22, step : 0.01, change : () => { this.matChanger(); }},
      { name : 'focalDepth', type : 'number', listen : true, min : 0.1, max : 100, step : 0.001, change : () => { this.matChanger(); }},
      { name : 'showFocus', type : 'checkbox', listen : true, change : () => { this.matChanger(); }}
    ]);
  }
  ...
  matChanger() {
    if (this.camera !== null) {
      const camera = this.camera.getCamera() as CinematicCamera;
      ...
      this.controls.focalDepth = postprocessing.bokeh_uniforms[ 'focalDepth' ].value;
    }
  } 
