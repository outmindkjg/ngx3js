...
export class WebglAnimationSkinningMorphComponent extends BaseComponent<{
  ...
  constructor() {
    super({
      status : 'Walking',
      jump : () => { this.fadeToAction('Jump', 0.2)},
      yes : () => { this.fadeToAction('yes', 0.2)},
      no : () => { this.fadeToAction('no', 0.2)},
      wave : () => { this.fadeToAction('wave', 0.2)},
      punch : () => { this.fadeToAction('punch', 0.2)},
      thumbsUp : () => { this.fadeToAction('thumbsUp', 0.2)},
      angry : 0,
      surprised : 0,
      sad : 0
    },[
      ...
      { name : 'Expressions' , type : 'folder' , children : [
        { name : 'angry', type : 'number', min : 0, max : 1, step : 0.01 , change : () => { 
          this.updateExpressions('Angry', this.controls.angry); 
        }},
        { name : 'surprised', type : 'number', min : 0, max : 1, step : 0.01 , change : () => { 
          this.updateExpressions('Surprised', this.controls.surprised) 
        }},
        { name : 'sad', type : 'number', min : 0, max : 1, step : 0.01 , change : () => { 
  ...
  fadeToAction(endAction : string , duration? : number, restoreAction? : string, restoreDuration? : number ) {
    if (this.mixer !== null) {
      restoreAction = ThreeUtil.getTypeSafe(restoreAction, this.controls.status);
