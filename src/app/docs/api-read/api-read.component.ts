...
export class ApiReadComponent implements OnInit {
  ...
  setBody(body: string, pageId: string) {
    ...
    const section = /\/(api|manual|examples|ngxapi|ngxmanual)\//.exec(pathname)[1].toString();
    ...
    switch (section) {
      ...
      case 'manual':
        ...
        break;
      case 'ngxapi':
        localizedPath = /\/ngxapi\/[A-z0-9\/]+/.exec(pathname).toString().substr(8);
        path = localizedPath.replace(/^[A-z0-9-]+\//, '');
        break;
      case 'ngxmanual':
        name = name.replace(/\-/g, ' ');
        path = pathname.replace(/\ /g, '-');
        path = localizedPath = /\/ngxmanual\/[-A-z0-9\/]+/.exec(path).toString().substr(11);
        break;
      case 'examples':
